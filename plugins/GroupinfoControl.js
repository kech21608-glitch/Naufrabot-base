...
module.exports = {
    ...
    async execute(sock, msg) {
        try {
            ...
            const baseDir = join('mhf', 'copy-group');
            ...
            if (action === 'لصق') {
                if (!name) {
                    return await sock.sendMessage(
                        ...
                        {
                            text:
                                '❗ اكتب اسم النسخة للصقها.\n' +
                                'مثال: .نسخة لصق مجموعة1'
                        },
                        { quoted: msg }
                    );
                }
                ...
                await sock.groupSettingUpdate(
                    groupJid,
                    data.settings.announce ? 'announcement' : 'not_announcement'
                );

                await sock.groupSettingUpdate(
                    groupJid,
                    data.settings.restrict ? 'locked' : 'unlocked'
                );
            ...
            if (action === 'حذف') {
                if (!name) {
                    return await sock.sendMessage(
                        groupJid,
                        { text: '❗ اكتب اسم النسخة لحذفها.\nمثال: .نسخة حذف مجموعة1' },
                        { quoted: msg }
                    );
                }
