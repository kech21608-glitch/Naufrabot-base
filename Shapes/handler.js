...
const chokidar = require('chokidar'); // لإضافة مراقبة الملفات
...
function startWatching() {
    ...
    watcher = chokidar.watch(pluginsPath, {
        ...
        ignoreInitial: true,
        awaitWriteFinish: {
            stabilityThreshold: 500, // انتظر 500ms بعد آخر تغيير
            pollInterval: 100
        }
    });
...
function handleMessagesLoader() {
    setTimeout(() => {
        if (!displayed && commands.size > 0) {
            ...
            // بدء مراقبة الملفات بعد تحميل البوت
            startWatching();
        }
    }, 50);
}
